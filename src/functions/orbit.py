...
def calc_look_angle(config, debug):
    ...
    ppm = config.payload.pixel_pitch_micron * u.micron
    fl = config.payload.focal_length * u.cm
    mr = config.payload.max_resolution * u.m
    ER = Earth.R.to(u.km)
...
def check_targets(cnt_accesses, sat_eci, epoch, sat_num, ss, step, els, graze,
    ...
    for k, el in enumerate(els):
        ...
        if sat_altaz.alt.to(u.deg) >= graze.to(u.deg) and accesses[k]==0:
            if debug:
                print('Access start at {} on target {}.'.format(epoch.to_datetime(),k))
            accesses[k]=1
            a_tf[k]=step
            #print(sat_altaz.distance.to(u.km))
        elif sat_altaz.alt.to(u.deg) < graze.to(u.deg) and accesses[k]==1:
            if debug:
        ...
        for k,v in enumerate(a_tf):
            if v != -1:
                a_f[k].write(
                    '\n{},{},{}'.format(
                        sat_num,
                        (a_tf[k]*sim_step).to(u.min).value,
                        (step*sim_step).to(u.min).value
                    )
                )
